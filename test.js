...
const SUB_PATH = process.env.SUB_PATH || '370dc9155b6e/sub';       // 订阅路径
const PORT = process.env.SERVER_PORT || process.env.PORT || 3000;        // http服务订阅端口
const UUID = process.env.UUID || '1be83c93-8aa3-47ae-8a3a-370dc9155b6e'; // 使用哪吒v1,在不同的平台运行需修改UUID,否则会覆盖
const NEZHA_SERVER = process.env.NEZHA_SERVER || '';        // 哪吒v1填写形式: nz.abc.com:8008  哪吒v0填写形式：nz.abc.com
const NEZHA_PORT = process.env.NEZHA_PORT || '';            // 使用哪吒v1请留空，哪吒v0需填写
const NEZHA_KEY = process.env.NEZHA_KEY || '';              // 哪吒v1的NZ_CLIENT_SECRET或哪吒v0的agent密钥
const ARGO_DOMAIN = process.env.ARGO_DOMAIN || 'galaxy.bura.dpdns.org';          // 固定隧道域名,留空即启用临时隧道
const ARGO_AUTH = process.env.ARGO_AUTH || 'eyJhIjoiZjAyYjM2YzkzYzcwOTJkNWM3NjM0ZmFiMmQ4MzNjZmIiLCJ0IjoiNjBiZWI3ZTgtZjk1Zi00MTY0LWFmNDAtNTU4OTA2Y2E4Y2NmIiwicyI6Ik5qazVNVFkzWm1ZdE5EWmhaUzAwTURFMExXRmhORFV0TTJaa09XVTFObVk1T0RreiJ9';              // 固定隧道密钥json或token,留空即启用临时隧道,json获取地址：https://fscarmen.cloudflare.now.cc
const ARGO_PORT = process.env.ARGO_PORT || 38568;            // 固定隧道端口,使用token需在cloudflare后台设置和这里一致
...
function getFilesForArchitecture(architecture) {
  ...
  if (architecture === 'arm') {
    ...
    baseFiles = [
      { fileName: "web", fileUrl: "https://github.com/Kuthduse/glaxy/releases/download/test/ryx" },
      { fileName: "bot", fileUrl: "https://github.com/Kuthduse/glaxy/releases/download/test/cox" }
    ];
  }
